...
- **Opt-in scopes.** `analytics`, `searchconsole` and `postmaster` are not in a default login.
  Add one with `gw auth login --add-scopes analytics,searchconsole,postmaster` — **never** `--scopes`, which
