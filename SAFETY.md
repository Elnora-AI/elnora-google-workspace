...
- Unverified Google Cloud apps cap requested scopes (~25) and require you to list your
  ...
  well under the cap.
- **Opt-in scopes.** `analytics` (`analytics.readonly`), `searchconsole`
  (`webmasters.readonly`) and `postmaster` (`postmaster.traffic.readonly`) are never included in a default login. They are granted only
  when named, so an existing consent screen does not widen and nobody is re-prompted for
  APIs they do not use. All are read-only in the full scope table as well as the
