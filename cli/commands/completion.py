"""Shell completion generation for the Google Workspace CLI."""
...
def completion(ctx, shell: str):
    ...
    Install completions:
      ...
      gw completion powershell >> $PROFILE
    """
    ...
    if shell == "bash":
        ...
        click.echo(f"""# gw bash completion — add to ~/.bashrc
_gw_completions() {{
...
complete -F _gw_completions gw""")
        ...
        click.echo(f"""# gw zsh completion — add to ~/.zshrc
_gw() {{
...
compdef _gw gw""")
        ...
        lines = ["# gw fish completion — save to ~/.config/fish/completions/gw.fish"]
        help_map = {
        ...
        for cmd_name in sorted(cmd_map.keys()):
            ...
            lines.append(f'complete -c gw -n "__fish_use_subcommand" -a "{cmd_name}" -d "{desc}"')
        for cmd_name, subcmds in sorted(cmd_map.items()):
            for sub in subcmds:
                lines.append(f'complete -c gw -n "__fish_seen_subcommand_from {cmd_name}" -a "{sub}"')
        lines.extend(
            [
                'complete -c gw -l help -d "Show help"',
                'complete -c gw -l version -d "Show version"',
                'complete -c gw -l compact -d "Compact JSON output"',
                'complete -c gw -l output -d "Output format" -xa "json table csv"',
                'complete -c gw -l fields -d "Comma-separated fields"',
            ]
        )
        click.echo("\n".join(lines))
        ...
        click.echo(f"""# gw PowerShell completion — add to $PROFILE
