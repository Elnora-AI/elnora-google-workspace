...
**Signature is appended by default** on every sending verb (the account's Gmail send-as signature). Pass `--no-signature` to omit it, or `--plain` on `send`, `draft` and the reply verbs to send plain text only (no HTML, signature or styling), or `--html-body HTML` on the same verbs to send your own HTML part verbatim (no signature). See **gw-gmail**.
